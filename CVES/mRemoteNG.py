#!/usr/bin/env python3
# mRemoteNG Password Decryption PoC
#
# Based on:
#   - https://github.com/haseebT/mRemoteNG-Decrypt/blob/master/mremoteng_decrypt.py
#   - https://www.hackthebox.com/machines/bastion
#
# Usage:
#     Option 1 (Windows, using 'type'):
#     1. On the target, open:
#          C:\Users\<username>\AppData\Roaming\mRemoteNG\confCons.xml
#     2. Find the <Password> field, copy the password.
#     3. Run on attacker machine:
#          python3 mremoteng_decrypt.py -s <ENCRYPTED_STRING>
#
#     Option 2 (whole file):
#     1. Copy confCons.xml from the target to your attacker machine
#        (via SMB, WinRM, or manual transfer).
#     2. Run:
#          python3 mremoteng_decrypt.py -f confCons.xml
#
# Default key is "mR3m". Use -p to supply a custom one if needed:
# python3 mremoteng_decrypt.py -s <ENCRYPTED_STRING> [-p CUSTOM_PASSWORD]
# python3 mremoteng_decrypt.py -f confCons.xml [-p CUSTOM_PASSWORD]
